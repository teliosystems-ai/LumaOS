...
| Signed model-catalog verification | Development-only bounded request-before-approval and detached-signature contract with three distinct production approvers, separate custodian, exact metadata/pack/runtime binding, lifecycle/revocation checks, and rollback checkpoint; no private-key handling, production trust loader, Admin event adapter, HSM ceremony, external anchor, or actual signed production catalog |
...
| G2 safety/catalog/admission boundary | 128 tests across installer, boot-state, helper, durable-effect, model-pack, model-selection, catalog-signing, host-inventory, and Ubuntu-preflight modules pass on native Windows and Ubuntu WSL under normal and optimized Python; both lanes share one physical host, and this is contract/readiness evidence only, not physical certification |
